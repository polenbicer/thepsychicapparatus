...
export default function Home() {
  ...
  const chooseConfession = (choice: string) => { setConfession(choice); addAnswer("Unsupervised Confession", "What is one thought about Polen you probably should not submit as evidence?", choice); bump({ id: 13, ego: 4 }); setScreen("dream"); };
  ...
  return <main className="game-shell"><div className="noise"/><section className="phone-frame">
    ...
    <div className="stage">
      ...
      {screen === "confession" && <div className="screen-in"><p className="chapter">08 / UNSUPERVISED CONFESSION</p><h2>What is one thought about Polen you probably should not submit as evidence?</h2><div className="choices"><button onClick={() => chooseConfession("I want to kiss her again")}>I want to kiss her again</button><button onClick={() => chooseConfession("I wonder what she would do if I stopped behaving")}>I wonder what she would do if I stopped behaving</button><button onClick={() => chooseConfession("I have replayed that kiss more than once")}>I have replayed that kiss more than once</button><button onClick={() => chooseConfession("The thought is too explicit for written evidence")}>The thought is too explicit for written evidence</button></div></div>}
      {screen === "dream" && <div className="screen-in confession"><p className="chapter">09 / DREAM ANALYSIS</p><h2>What is the last dream you remember?</h2><p className="microcopy">Fragments, nonsense and suspicious cameos are admissible. Spicy material is not only admissible; it may be diagnostically significant.</p><textarea value={dream} onChange={(e) => setDream(e.target.value)} maxLength={800} placeholder="I remember…"/><div className="character-count">{dream.length}/800</div><button className="primary" disabled={!dream.trim()} onClick={submitDream}>SUBMIT TO THE UNCONSCIOUS</button></div>}
